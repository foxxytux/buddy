...
```bash
npm install @mariozechner/buddy-coding-agent
```
...
The `Agent` class (from `@mariozechner/buddy-agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
import { AuthStorage, ModelRegistry } from "@mariozechner/buddy-coding-agent";
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@mariozechner/buddy-coding-agent";
...
import { createAgentSession, defineTool } from "@mariozechner/buddy-coding-agent";
...
```typescript
import { createEventBus, DefaultResourceLoader } from "@mariozechner/buddy-coding-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@mariozechner/buddy-coding-agent";
