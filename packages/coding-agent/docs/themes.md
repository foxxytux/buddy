> buddy can create themes. Ask it to build one for your setup.
...
Buddy loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.buddy/agent/themes/*.json`
- Project: `.buddy/themes/*.json`
- Packages: `themes/` directories or `buddy.themes` entries in `package.json`
...
On first run, buddy detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.buddy/agent/themes
vim ~/.buddy/agent/themes/my-theme.json
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/badlogic/buddy-mono/main/packages/coding-agent/src/modes/interactive/theme/theme-schema.json",
  "name": "my-theme",
  "vars": {
    "primary": "#00aaff",
    "secondary": 242
...
**Hot reload:** When you edit the currently active custom theme file, buddy reloads it automatically for immediate visual feedback.
...
Buddy uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, buddy falls back to the nearest approximation.
