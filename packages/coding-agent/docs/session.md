...
```
~/.buddy/agent/sessions/--<path>--/<timestamp>_<uuid>.jsonl
```
...
Sessions can be removed by deleting their `.jsonl` files under `~/.buddy/agent/sessions/`.

Buddy also supports deleting sessions interactively from `/resume` (select a session and press `Ctrl+D`, then confirm). When available, buddy uses the `trash` CLI to avoid permanent deletion.
...
Source on GitHub ([buddy-mono](https://github.com/badlogic/buddy-mono)):
- [`packages/coding-agent/src/core/session-manager.ts`](https://github.com/badlogic/buddy-mono/blob/main/packages/coding-agent/src/core/session-manager.ts) - Session entry types and SessionManager
- [`packages/coding-agent/src/core/messages.ts`](https://github.com/badlogic/buddy-mono/blob/main/packages/coding-agent/src/core/messages.ts) - Extended message types (BashExecutionMessage, CustomMessage, etc.)
...
### Base Message Types (from buddy-ai)
...
### Extended Message Types (from buddy-coding-agent)
...
- `fromHook`: `true` if generated by an extension, `false`/`undefined` if buddy-generated (legacy field name)
...
Session metadata (e.g., user-defined display name). Set via `/name` command or `buddy.setSessionName()` in extensions.
