...
import type { ExtensionAPI, SessionBeforeSwitchEvent, SessionMessageEntry } from "@foxxytux/buddy-coding-agent";

export default function (buddy: ExtensionAPI) {
	buddy.on("session_before_switch", async (event: SessionBeforeSwitchEvent, ctx) => {
		if (!ctx.hasUI) return;
