...
- Exported `CustomProviderDialog` from `@foxxytux/buddy-web-ui` ([#2267](https://github.com/badlogic/pi-mono/issues/2267))
...
- Build `@foxxytux/buddy-web-ui` with `tsc` instead of `tsgo` so Lit decorator-based state updates rerender correctly.
...
- **Agent class moved to `@foxxytux/buddy-agent-core`**: The `Agent` class, `AgentState`, and related types are no longer exported from this package. Import them from `@foxxytux/buddy-agent-core` instead.
...
- **`AppMessage` renamed to `AgentMessage`**: Now imported from `@foxxytux/buddy-agent-core`. Custom message types use declaration merging on `CustomAgentMessages` interface.
...
- **`CustomMessages` interface removed**: Use declaration merging on `CustomAgentMessages` from `@foxxytux/buddy-agent-core` instead.

- **`agent.appendMessage()` removed**: Use `agent.queueMessage()` instead.

- **Agent event types changed**: `AgentInterface` now handles new event types from `@foxxytux/buddy-agent-core`: `message_start`, `message_end`, `message_update`, `turn_start`, `turn_end`, `agent_start`, `agent_end`.
...
- `Agent` class (moved to `@foxxytux/buddy-agent-core`)
...
**Before (0.30.x):**
```typescript
import { Agent, ProviderTransport, type AppMessage } from '@foxxytux/buddy-web-ui';
...
import { defaultConvertToLlm } from '@foxxytux/buddy-web-ui';
...
declare module "@foxxytux/buddy-web-ui" {
