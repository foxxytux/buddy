...
```bash
npm install @foxxytux/buddy-mom
```
...
- run interactive coding agent session: `npx @foxxytux/buddy-coding-agent`
- enter `/login` command
...
Update mom anytime with `npm install -g @foxxytux/buddy-mom`. This only updates the Node.js app on your host. Anything mom installed inside the Docker container remains unchanged.
