...
```bash
npm install @mariozechner/buddy-mom
```
...
# Option 2: use /login command in pi agent, then copy/link auth.json to ~/.buddy/mom/
...
- run interactive coding agent session: `npx @mariozechner/buddy-coding-agent`
- enter `/login` command
  - choose "Anthropic" provider
  - follow instructions in the browser
- link `auth.json` to mom: `ln -s ~/.buddy/agent/auth.json ~/.buddy/mom/auth.json`
...
Update mom anytime with `npm install -g @mariozechner/buddy-mom`. This only updates the Node.js app on your host. Anything mom installed inside the Docker container remains unchanged.
