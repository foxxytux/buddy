...
```typescript
import { type AgentSessionEvent } from "@mariozechner/buddy-coding-agent";
...
import type { MomCustomTool, ToolAPI } from "@mariozechner/buddy-mom";
...
Tools are loaded via jiti. They can import any 3rd party dependencies (install in the tool directory). Imports of `@mariozechner/buddy-ai` and `@mariozechner/buddy-mom` are aliased to the running mom bundle.
