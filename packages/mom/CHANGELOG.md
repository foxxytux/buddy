...
- `AgentTool` import moved from `@foxxytux/buddy-ai` to `@foxxytux/buddy-agent-core`
...
- Support for OAuth login via coding agent's `/login` command (link `~/.buddy/agent/auth.json` to `~/.buddy/mom/auth.json`)
